...
- Optional per-notification contracts: `NotificationWithID`, `NotificationWithShouldSend`, `NotificationWithAfterSending`, and `NotificationWithDatabaseConnection`, plus the notifiable-side typed routing contracts `MailRoutable` (`RouteNotificationForMail`) and `DatabaseRoutable` (`RouteNotificationForDatabase`).
- Built-in channel names are available as the `notification.ChannelMail` and `notification.ChannelDatabase` constants.
