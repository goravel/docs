...
- [Fix migrations with the non-default connection writing the ledger row to the wrong database](#fix-migrations-with-the-non-default-connection-writing-the-ledger-row-to-the-wrong-database)
...
::: warning
Wildcard rules (e.g., `scores.*`, `tags.*`) now consistently appear in error output even when no input is provided. Add assertions for these if your tests check the full error response body.
:::
...
::: tip
If you want to use this feature in the fiber driver, please check [Fiber driver template configuration replaced with DefaultTemplate](#fiber-driver-template-configuration-replaced-with-defaulttemplate).
:::
...
Custom middleware must be updated to implement the new interface. See the [upgrade guide](#upgrade-guide) for migration steps.
...
### Fix migrations with the non-default connection writing the ledger row to the wrong database

goravel/framework: v1.18.1

Previously, when two or more migrations in the same batch returned the same non-default connection from `Connection()`, the second migration's ledger row was recorded on that connection instead of the default one, causing the migration to fail with a `table doesn't exist` error. Now each migration still runs on its declared connection, while its ledger row is recorded on the default connection, matching Laravel's behavior.

Issue: [#987](https://github.com/goravel/goravel/issues/987)

### Fix package views registered in Boot() not rendering

goravel/gin: v1.18.1

goravel/fiber: v1.18.1

Previously, the default template set was compiled when the route engine was built (`init()`), which runs before service providers boot. Views registered via `facades.View().LoadViewsFrom()` in a provider's `Boot()` were therefore missing from the compiled set, and rendering them at request time failed. Now the default template set is compiled lazily on the first serve call (`Run`/`Listen`/`ServeHTTP`), after all providers have booted, so `LoadViewsFrom()`-registered views are picked up.

Issue: [#989](https://github.com/goravel/goravel/issues/989)
