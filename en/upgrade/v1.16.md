...
Fix the issue where daily log rotation did not work correctly, the new day log file was not created except restarting the application.
...
Enhance the database row scanning functionality to properly handle JSON columns and custom types. Added new decode hooks for automatic JSON-to-Go conversions, supporting scanning JSON columns into `map[string]any`, slices (`[]string`, `[]int`), and custom structs.
