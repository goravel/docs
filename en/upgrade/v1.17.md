...
- [Fix delay task issue when using redis queue driver](#fix-delay-task-issue-when-using-redis-queue-driver)
...
### Fix delay task issue when using redis queue driver

goravel/framework: v1.17.1

When using the redis queue driver and multiple concurrency, the delay task will be executed multiple times. Now the delay task is fixed to ensure it only executes once.
