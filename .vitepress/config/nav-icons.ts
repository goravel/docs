...
/* Render an Iconify icon as an inline svg. Brand marks keep their owner's colour. */
function icon(set: IconifyJSON, name: string, color?: string): string {
  const { attributes, body } = iconToSVG(getIconData(set, name)!)
  const style = color ? ` style="color:${color}"` : ''
  return iconToHTML(body, { ...attributes, class: `g-menu-icon${color ? ' is-brand' : ''}` }).replace('<svg', `<svg${style}`)
}
...
export const ICON_CONTRIBUTE = icon(carbon, 'pull-request')
export const ICON_TRANSLATE = icon(carbon, 'translate')
export const ICON_SUPPORT = icon(carbon, 'favorite')

// DevChalk's mark is a painting, so it is an image rather than an icon
export const ICON_DEVCHALK =
  '<img class="g-menu-icon is-avatar" src="/devchalk-mark.png" alt="" width="20" height="20" />'
