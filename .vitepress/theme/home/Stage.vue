...
const route = computed(() => {
  ...
  const along = (i: number) => polylineLength(main.slice(0, i + 1))
...
/* The stop the request has reached, when the view is following one. */
const at = computed(() => (props.view.kind === 'journey' ? props.view.stop : undefined))

/* How much of the route is still ahead of the request, as a dash offset. */
const mainOffset = computed(() => route.value.mainLen - (at.value ? route.value.reach[at.value] : 0))
const branchOffset = computed(() => (at.value === 'event' || at.value === 'resp' ? 0 : route.value.branchLen))
const headAt = computed(() => (at.value ? route.value.head[at.value] : route.value.p.core))
...
<template>
  ...
  >
    ...
    <g class="g-request" :class="{ 'is-on': at }">
