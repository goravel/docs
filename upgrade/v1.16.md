...
If you are not using it, you need to modify the configuration in the `config/queue.go` file:
...
[View Document](../security/authentication.md)
...
The files generated by the `make:migration` and `make:seeder`, etc. commands will be automatically registered to the framework after generation, no longer need to manually register like before.
