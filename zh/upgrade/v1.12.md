...
- [修改 facades 调用方式](#5--修改-facades-*)
- [优化 Artisan Flag 参数格式](#6--修改-artisan-命令中的-flag（如果用到）)
- [Filesystem 模块取消对 S3, OSS, COS, Minio 驱动的默认支持](#7--filesystem-模块取消对-s3--oss--cos--minio-驱动的默认支持)
- [Cache 模块取消对 Redis 驱动的默认支持](#8--cache-模块取消对-redis-驱动的默认支持)
...
删除 `config/filesystems.go::disks` 中除 `local` 以外的键：
...
删除 `config/cache.go::stores` 中的 `redis` 键：
...
时间处理更优雅，[详见文档](../digging-deeper/helpers.md#时间)。
