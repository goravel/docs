...
- [移除默认加载的 HTTP 全局中间件](#5--移除默认加载的-http-全局中间件)
- [优化 Controller 返回](#6-优化-controller-返回)
- [优化 facades.Cache() 中 Remember 与 RememberForever 方法](#7--修改-facades-cache-中-remember-与-rememberforever-方法（如果用到）)
- [移除废弃方法](#8-移除废弃方法（如果用到）)
...
### 7. 修改 facades.Cache() 中 Remember 与 RememberForever 方法（如果用到）
...
1. `Request` 的 `Form` 和 `Json` 方法，使用 `Input` 方法替代；
2. `Log` 自定义驱动的 `GetLevel`, `GetTime`, `GetMessage` 方法，使用 `Level`, `Time`, `Message` 方法替代；
3. `gorm.New` 方法，该方法用于直接获取 `gorm` 实例，已不推荐使用，如有必要，请使用 `gorm.NewGormImpl` 方法替代；
