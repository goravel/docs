...
- [Schedule 新增 Stop 方法](#schedule-新增-stop-方法)
- [HTTP 支持设置超时时间](#http-支持设置超时时间)
- [错误日志输出的 trace 支持点击跳转](#错误日志输出的-trace-支持点击跳转)
- [多个 Auth.Guard 可以设置自己的 TTL](#多个-authguard-可以设置自己的-ttl)
...
- [Route 的 Shutdown 方法重命名为 Stop](#route-的-shutdown-方法重命名为-stop)
- [优化 Cache 的 Decrement 与 Increment 方法](#优化-cache-的-decrement-与-increment-方法)
- [优化 Artisan 的 Call 方法](#优化-artisan-的-call-方法)
- [Testing.Database 的 Clear 方法重命名为 Stop](#testing-database-的-clear-方法重命名为-stop)
- [优化 Testing.Database 的 Build 方法](#优化-testing-database-的-build-方法)
- [优化 Request.Input* 方法](#优化-requestinput-方法)
- [优化 validation.PrepareForValidation 方法](#优化-validationprepareforvalidation-方法)
...
### 6. 如果正在使用 Orm.Transaction 方法

需相应修改：[修改 Orm.Transaction 方法回调参数类型](#修改-orm-transaction-方法回调参数类型)

### 7. 如果正在使用 Orm 的 Delete 或 ForceDelete 方法

需相应修改：[优化 Orm 的 Delete 与 ForceDelete 方法](#优化-orm-的-delete-与-forcedelete-方法)

### 8. 如果正在使用 Route 的 Shutdown 方法

需相应修改：[Route 的 Shutdown 方法重命名为 Stop](#route-的-shutdown-方法重命名为-stop)

### 9. 如果正在使用 Cache 的 Decrement 或 Increment 方法

需相应修改：[优化 Cache 的 Decrement 与 Increment 方法](#优化-cache-的-decrement-与-increment-方法)

### 10. 如果正在使用 Artisan 的 Call 方法

需相应修改：[优化 Artisan 的 Call 方法](#优化-artisan-的-call-方法)

### 11. 如果正在使用 Route 的 Shutdown 方法

需相应修改：[Route 废弃 Shutdown 方法](#route-废弃-shutdown-方法)

### 12. 如何正在使用 Testing.Database 的 Clear 方法

需相应修改：[Testing.Database 的 Clear 方法重命名为 Stop](#testing-database-的-clear-方法重命名为-stop)

### 13. 如果正在使用 Testing.Database 的 Build 方法

需相应修改：[优化 Testing.Database 的 Build 方法](#优化-testing-database-的-build-方法)

### 14. 如果正在使用 Migration 模块
...
### 15. 如果正在使用 Request.Input* 方法

需相应修改：[优化 Request.Input* 方法](#优化-request-input-方法)

### 16. 如果正在使用 validation.PrepareForValidation 方法
...
### Route 的 Shutdown 方法重命名为 Stop

goravel/framework: v1.15.0

为了保持 `Stop` 方法名称与其他模块一致，我们将 `Shutdown` 方法重命名为 `Stop`。
...
### Testing.Database 的 Clear 方法重命名为 Stop

goravel/framework: v1.15.0

为了保持 `Stop` 方法名称与其他模块一致，我们将 `Clear` 方法重命名为 `Stop`。
