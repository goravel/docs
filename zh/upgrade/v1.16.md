...
- [重构 facades.Queue() 支持更多特性](#重构-facades-queue-支持更多特性)
...
- [installer 支持升级](#installer-支持升级)
- [优化 Request 超时逻辑](#优化-request-超时逻辑)
- [多语言文件支持 embed 加载](#多语言文件支持-embed-加载)
- [支持根据数据表创建 Model](#支持根据数据表创建-model)
- [定时任务支持设置秒级任务](#定时任务支持设置秒级任务)
- [Request 支持绑定多个文件](#request-支持绑定多个文件)
- [Request 支持获取原始路由路径](#request-支持获取原始路由路径)
- [Route 新增获取所有路由方法](#route-新增获取所有路由方法)
- [Route 支持设置路由名称](#route-支持设置路由名称)
- [Testing 支持创建 Cache 镜像](#testing-支持创建-cache-镜像)
- [Orm 支持操作 JSON 字段](#orm-支持操作-json-字段)
- [Orm 支持设置 Global Scope](#orm-支持设置-global-scope)
- [不同 JWT Guard 支持设置不同的 secert 与 ttl](#不同-jwt-guard-支持设置不同的-secert-与-ttl)
- [新增 route:list 命令](#新增-route-list-命令)
...
- 如果没有使用或想要使用 database 驱动，需要修改 `config/queue.go` 文件中的配置：
...
添加 database 驱动需要的迁移文件： [database/migrations/20210101000002_create_jobs_table.go](https://github.com/goravel/goravel/blob/master/database/migrations/20210101000002_create_jobs_table.go)
...
### 5. 如果正在使用 session 自定义驱动

需相应修改：[优化 Session 自定义驱动方式](#优化-session-自定义驱动方式)

### 5. 如果正在使用 facades.Config() 中的方法
...
### 重构 facades.Queue() 支持更多特性

1. v1.15 之前 Queue 仅支持同步驱动与 Redis 驱动，从 v1.16 开始支持数据库驱动及自定义驱动。
2. 新增 `jobs` 和 `failed_jobs` 表，用于存储任务及失败任务。
3. 新增 `queue:retry` 命令，用于重试失败的任务。
...
当设置 `http.request_timeout` 配置项 为 `0` 时，请求将不会超时，支持等待。
...
支持更新 JSON 字段，[查看文档](../orm/getting-started.md#更新json-字段)。

### Orm 支持设置 Global Scope
...
[查看文档](../security/authentication.md#不同-jwt-guard-支持不同配置)。
...
1. 优化 `Count`, `Distinct`, `Select`, `Exists`, `Sum` 方法：
