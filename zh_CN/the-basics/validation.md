...
该命令生成的表单请求类将被置于 `app/http/requests` 目录中。 如果这个目录不存在，在你运行 `make:request` 命令后将会创建这个目录。 Goravel 生成的每个表单请求都有 `Authorize` 和 `Rules` 方法。 你还可以自定义可选的 `Filters`、`Messages`、`Attributes` 和 `PrepareForValidation` 方法以进行进一步操作。
...
你可以通过完善表单请求的 `Filters` 方法来格式化输入数据。 此方法应返回 `attribute/filter` 对的映射。 筛选器值可以是字符串或 `[]string` 值：
...
如果你不想在请求中使用 `Validate` 方法，你可以使用 `facades.Validation()` 手动创建一个验证器实例。 facades 中的 `Make` 方法将会生成一个新的验证器实例：
...
在 `ctx` 之后，传递给 `Make` 方法的数据参数可以是 `map[string]any`、`struct`、`url.Values`、`map[string][]string`、`*http.Request` 或其他支持的请求数据源。 下一个参数是一个验证规则的 `map[string]any`。 规则值可以是字符串或 `[]string` 值。
...
显式消息覆盖优先于自定义规则默认值。 Goravel 按以下顺序解析消息：`field.rule` 消息，然后 `rule` 消息，最后是自定义规则的 `Message()` 返回值。

### 指定自定义属性值

Goravel 的许多内置验证规则错误消息都包含 `:attribute` 占位符。 要自定义用于替换特定字段占位符的值，你可以使用 `validation.Attributes` 传递自定义属性（也适用于 `ctx.Request().Validate()`）：
...
你可以在验证数据前先格式化数据，以便更灵活地进行数据验证，你可以使用 `validation.PrepareForValidation` 传递格式化回调（也适用于 `ctx.Request().Validate()`）：
...
在使用表单请求或手动创建的验证器实例验证传入请求数据后，你可以检索已验证的数据或将请求数据绑定到 `struct`。

1. 验证通过后，使用 `Validated` 方法仅检索验证规则涵盖的字段。 排除的字段会被省略，通配符切片规则保持原始切片形状：
...
2. 验证成功后，使用 `Bind` 方法将数据绑定到结构体。 `Bind` 将验证后的数据合并回原始请求数据，因此没有规则的字段仍然可以绑定：
...
3. 使用「表单请求」进行验证时，传入的数据将会自动被绑定到表单：
...
验证错误消息现在由框架的[本地化](../digging-deeper/localization.md)系统提供。 默认消息嵌入在框架中，并自动使用应用程序的当前区域设置。 消息查找遵循以下顺序：

1. 表单请求 `Messages()` 或显式 `validation.Messages()` 覆盖（`field.rule`，然后是 `rule`）
2. 自定义规则的 `Message()` 方法
3. 使用 `validation.*` 键进行翻译文件查找（例如 `validation.required`、`validation.min.string`）
4. 嵌入的默认消息

对于基于大小的规则，如 `min`、`max`、`between`、`size`、`gt`、`gte`、`lt` 和 `lte`，翻译键包含属性类型后缀（例如 `min.string`、`min.numeric`、`min.array`、`min.file`）。
...
这将验证翻译文件从框架复制到 `lang/en/validation.json`，您可以在其中自定义默认错误消息。
...
发布后，编辑 `lang/en/validation.json` 以自定义任何验证消息。 例如：

```json
{
    "required": "您忘记了 :attribute 字段！",
    "email": ":attribute 必须是有效的电子邮件地址。"
}
```

这些自定义消息将应用于应用程序中的所有验证器，除非被表单请求 `Messages()` 或显式 `validation.Messages()` 覆盖。

### 翻译成其他语言

为了支持其他语言，在相应的语言目录中创建已发布的 `lang/en/validation.json` 的副本（例如 `lang/zh/validation.json`）并翻译消息。 框架将根据由 `facades.App().SetLocale(ctx, "zh")` 设置的当前区域设置使用相应的语言文件。

## 可用的验证规则

验证规则名称现在默认使用 snake_case。 规则和过滤器使用 `map[string]any` 定义；每个值必须是管道分隔的 `string` 或规则名称的 `[]string`。
...
当 `regex` 或 `not_regex` 模式包含 `|` 并且需要在之后添加更多规则时，使用 `[]string`。
...
| 已接受 / 已拒绝 | `accepted`, `accepted_if`, `declined`, `declined_if`                                                                                                                                                                                  |
...
| 数字        | `digits`, `digits_between`, `decimal`, `multiple_of`, `min_digits`, `max_digits`                                                                                                                                                      |
...
The `size`、`min`、`max`、`between`、`gt`、`gte`、`lt` 和 `lte` 规则是类型感知的。 它们比较数字字段的数值、字符串的字符串长度、数组、切片和映射的元素数量，以及文件的大小。
...
`active_url` 规则对 URL 主机执行 DNS 查找。 请在请求热路径上小心使用，因为 DNS 解析可能会增加延迟。

### 已弃用的规则别名

以下别名保持向后兼容，但将在下一个主要版本中移除。 建议使用新的 snake_case 名称：
...
func (receiver *Between) Message(ctx context.Context) string {
  return ":attribute 值 :value 必须在 :option0 和 :option1 之间。"
}
```
...
验证过滤器默认使用蛇形命名法（snake_case）。 过滤器在验证之前运行，可以在表单请求中声明，或者在手动创建验证器时使用 `validation.Filters` 声明。
...
| 简短类型别名  | `int`, `int64`, `uint`, `float`, `bool`                                        |
...
以下别名保持向后兼容，但将在下一个主要版本中移除。 请使用新的 snake_case 名称：
...
要生成一个新的过滤器对象，你可以使用 `make:filter` Artisan 命令。 让我们使用这个命令生成一个将字符串转换为整数的过滤器。 这个过滤器框架已经内置，这里只是为了示例。 Goravel 会将新规则放在 `app/filters` 目录中。 如果此目录不存在，Goravel 将在你执行 Artisan 命令创建过滤器时创建它：
...
通过 `make:filter` 创建的新过滤器将自动注册到 `bootstrap/filters.go::Filters()` 函数中，并且该函数将由 `WithFilters` 调用。 如果你自行创建过滤器文件，则需要手动注册过滤器。
