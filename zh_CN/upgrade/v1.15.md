...
随着 [Golang v1.21 不再被维护](https://endoflife.date/go)，Goravel v1.15 默认支持 Golang 版本由 1.21 升级为 1.22。 请在 go.mod 文件中更新版本。 请在 go.mod 文件中更新版本。
...
之前框架仅支持 SQL 迁移， 当想要切换数据库时，由于不同数据库间的 SQL 语法差异，将使迁移过程极其困难。 此外，迁移文件中也无法执行代码，在进行数据修复时，无法进行逻辑判断。 当想要切换数据库时，由于不同数据库间的 SQL 语法差异，将使迁移过程极其困难。 此外，迁移文件中也无法执行代码，在进行数据修复时，无法进行逻辑判断。
...
之前多个 Guard 公用 `jwt.ttl` 配置。 之前多个 Guard 公用 `jwt.ttl` 配置。 现在你可以通过在 `config/auth.go` 文件中为每个 Guard 单独设置 TTL。 如果不设置，则默认使用 `jwt.ttl` 配置。 如果不设置，则默认使用 `jwt.ttl` 配置。
...
Postgresql 驱动的名称已更改为 `postgres`。 如果你正在使用 Postgresql 驱动程序，则需要修改配置文件： 如果您正在使用 Postgresql 驱动程序，则需要修改配置文件：
...
之前 `facades.Artisan().Call()` 方法执行时遇到错误将会直接 panic。 现在将返回错误，如果你正在使用该方法，请处理该错误返回。 现在将返回错误，如果你正在使用该方法，请处理该错误返回。
...
之前在调用 `Build` 方法时，会自动进行数据库迁移。 之前在调用 `Build` 方法时，会自动进行数据库迁移。 升级后，需要手动调用 `Migrate` 方法进行迁移，数据库控制更加灵活。
...
之前 `Request.Input*` 方法仅能获取 `Body` 中的数据，现在将依次查找 `Body`、 `Query`、`Param`。 并且之前向 `Request.Input*` 方法传递第二个参数(默认值)时，如果 `key` 存在，但置为空，将返回默认值。 现在将返回空字符串，因为空字符串也是一种有效值，只有当 key 不存在时，才返回默认值。 并且之前向 `Request.Input*` 方法传递第二个参数(默认值)时，如果 `key` 存在，但置为空，将返回默认值。 现在将返回空字符串，因为空字符串也是一种有效值，只有当 key 不存在时，才返回默认值。
...
之前 Artisan log 仅输出 error 级别的日志，现在将输出所有级别的日志。 之前 Artisan log 仅输出 error 级别的日志，现在将输出所有级别的日志。 如果想禁止 console 打印日志且正在使用 `single` 或 `daily` 日志驱动，请将 `logging.channels.single.print` 或 `logging.channels.daily.print` 配置设置为 `false`。
...
修复前，`PrepareForValidation` 方法中 `ctx` 始终为 `nil`，现在将正确传递 `ctx` 参数。 如果你正在使用 `validation.PrepareForValidation` 方法，请相应修改。 如果你正在使用 `validation.PrepareForValidation` 方法，请相应修改。
...
[dromara/carbon](https://github.com/dromara/carbon) ~v2.5.5 以上版本引入了新的特性，导致框架部分功能异常。 [dromara/carbon](https://github.com/dromara/carbon) ~v2.5.5 以上版本引入了新的特性，导致框架部分功能异常。 注意，当前尚不支持 dromara/carbon v2.6 以上版本，将在 Goravel v1.16 中兼容。
...
之前一直返回 `127.0.0.1`。 之前一直返回 `127.0.0.1`。 修复后，新增了 `http.drivers.fiber.proxy_header`, `http.drivers.fiber.enable_trusted_proxy_check` 和 `http.drivers.fiber.trusted_proxies` 配置，用于设置代理头与可信任的代理 IP 地址。
