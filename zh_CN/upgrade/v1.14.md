...
Goravel v1.14 基于 Golang 1.21 进行开发与测试，与其他低 Golang 版本一般情况下可以兼容运行。 请根据本节内容，一步步进行框架升级。 请根据本节内容，一步步进行框架升级。
...
检查是否在校验数据失败的情况下依然尝试读取绑定值。 如果有，需优化逻辑，因为该值升级后为空。 详见：[Validation 如果验证失败将不绑定数据](#validation-如果验证失败将不绑定数据)。 如果有，需优化逻辑，因为该值升级后为空。 详见：[Validation 如果验证失败将不绑定数据](#validation-如果验证失败将不绑定数据)。
...
之前调用 `Parse`, `User`, `Login`, `LoginUsingID`, `Refresh`, `Logout` 方法时，需要传递 `ctx`。 之前调用 `Parse`, `User`, `Login`, `LoginUsingID`, `Refresh`, `Logout` 方法时，需要传递 `ctx`。 升级后不再需要，而是可以直接设置在 `facades.Auth(ctx)`。
