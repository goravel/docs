...
- [修复 Orm.Connection() 缓存路径污染 dbConfig 的问题](#修复-orm-connection-缓存路径污染-dbconfig-的问题)
...
### 修复 Orm.Connection() 缓存路径污染 dbConfig 的问题

goravel/framework: v1.18.1

之前，`facades.Orm().Connection(name)` 存在一个缓存快速路径，会忽略请求的连接并复用默认连接的数据库配置，因此 `Name()`、`DatabaseName()` 以及依赖连接作用域的行为（例如迁移记录写入的位置）仍然指向默认连接。在同一个非默认连接上连续运行两个迁移时，第二条迁移记录会被写入错误的数据库。现在，每个连接会从共享的、并发安全的 `QueriesCache` 中解析自己的配置。
