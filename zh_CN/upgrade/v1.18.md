...
- [修复使用非默认连接的迁移将迁移记录写入错误数据库的问题](#修复使用非默认连接的迁移将迁移记录写入错误数据库的问题)
- [修复在 Boot() 中注册的包视图无法渲染的问题](#修复在-boot-中注册的包视图无法渲染的问题)
...
::: warning
通配符规则（如 `scores.*`、`tags.*`）现在即使没有提供输入也会一致地出现在错误输出中。如果你的测试检查完整的错误响应体，请为之添加断言。
:::
...
::: warning
中间件排除使用 `Signature()` 方法来识别中间件。确保每个中间件返回唯一的签名，以便 `WithoutMiddleware` 正常工作。内置框架中间件已提供唯一签名。
:::

[查看文档](../the-basics/routing.md#排除中间件)
...
[查看文档](../digging-deeper/artisan-console.md#命令过滤)
...
::: tip
如果要在 Fiber 驱动中使用此功能，请参阅 [Fiber 驱动模板配置替换为 DefaultTemplate](#fiber-驱动模板配置替换为-defaulttemplate)。
:::
...
自定义中间件必须更新为实现新接口。迁移步骤请参阅[升级指南](#升级指南)。
...
### 修复使用非默认连接的迁移将迁移记录写入错误数据库的问题

goravel/framework: v1.18.1

之前，当同一批次中的两个或多个迁移通过 `Connection()` 返回相同的非默认连接时，第二条迁移记录会被写入该连接而非默认连接，导致迁移因 `table doesn't exist` 错误而失败。现在，每个迁移仍在其声明的连接上运行，而迁移记录会被记录在默认连接上，与 Laravel 的行为一致。

Issue: [#987](https://github.com/goravel/goravel/issues/987)

### 修复在 Boot() 中注册的包视图无法渲染的问题

goravel/gin: v1.18.1

goravel/fiber: v1.18.1

之前，默认模板集在路由引擎构建时（`init()`）编译，而该时机早于服务提供者启动。因此，在服务提供者的 `Boot()` 中通过 `facades.View().LoadViewsFrom()` 注册的视图不会包含在编译集中，在请求时渲染这些视图会失败。现在，默认模板集会在首次提供服务时（`Run`/`Listen`/`ServeHTTP`）延迟编译，此时所有服务提供者都已启动，`LoadViewsFrom()` 注册的视图会被正确加载。
