...
- [修复 Schema 指定驱动无效的问题](#修复-schema-指定驱动无效的问题)
- [修复当使用 WithMiddleware() 时 ServiceProvider.Boot() 中注册的路由被静默丢弃的问题](#修复当使用-withmiddleware-时-serviceprovider-boot-中注册的路由被静默丢弃的问题)
- [修复 Request InputArray 将单个值按空格分割的问题](#修复-request-inputarray-将单个值按空格分割的问题)
- [修复模块路径导入错误的问题](#修复模块路径导入错误的问题)
- [修复 LOG_LEVEL 无效的问题](#修复-log-level-无效的问题)
...
之前，`facades.Schema().Connection(name)` 使用了指定的连接，但仍然会使用默认连接的驱动来编译 schema 查询。 现在，`HasTable` 和 `db:wipe --database` 等操作将使用所选择连接的驱动。

Issue: [#942](https://github.com/goravel/goravel/issues/942)

### 修复当使用 WithMiddleware() 时 ServiceProvider.Boot() 中注册的路由被静默丢弃的问题

goravel/framework: v1.17.3

以前，当通过 `bootstrap/app.go` 中的 `WithMiddleware()` 注册全局中间件时，`configureMiddleware()` 在 `Boot()` 之后运行，这会重建路由引擎并丢弃服务提供者在 `Boot()` 期间注册的任何路由。 现在，`configureMiddleware()` 被移到 `Register()` 和 `Boot()` 之间运行，确保提供者注册的路由在引擎重建后仍然存在。

Issue: [#975](https://github.com/goravel/goravel/issues/975)

### 修复 Request InputArray 将单个值按空格分割的问题

goravel/gin: v1.17.1

goravel/fiber: v1.17.1

以前，`ctx.Request().InputArray` 可能会将包含空格的单个表单或url编码的数组值分割成多个条目。 现在，单个提交的值（如 `123 456 789`）保持为一个数组元素，而多值数组则保持现有行为。
...
以前，像 `make:migration`、`make:test` 和包只使用多段 `go.mod` 模块路径的最后一段来生成导入，例如 `github.com/example/project/app/facades` 只会导入 `facades`。 现在生成的导入使用完整的模块路径。
...
以前，设置 `LOG_LEVEL=error` 没有效果，Info 和 Debug 消息仍然被写入，因为 `Writer.log()` 绕过了级别检查。 现在，在写入任何日志条目之前，会正确检查配置的 `LOG_LEVEL`。
