...
### 6. 如果您开发了自定义包并通过 `package:install` 命令安装它

需相应修改：[修改包设置方法](#修改包设置方法)
...
我们很高兴推出 [Goravel Lite](https://github.com/goravel/goravel-lite)，这是 Goravel 框架的轻量级版本，专为希望以最小依赖和更小占用空间构建应用程序的开发者设计。 它提供了 Goravel 的核心功能，同时允许开发者仅添加他们需要的组件。

- 仅包含必要的 facades：`Artisan`、`Config`，减少体积并提高性能。
- 其他 facades 可通过 `package:install` 和 `package:uninstall` 命令按需安装和卸载。
- `goravel/goravel` == `goravel/goravel-lite` + 所有 facades。 对 `goravel/goravel` 的任何改进都应提交到 `goravel/goravel-lite`，然后 GitHub Action 会将更改同步到 `goravel/goravel`。 `goravel/goravel` 仍然是用户使用 Goravel 框架的主要仓库。
...
以前，运行 `package:install` 命令时，包服务提供者可以通过 `setup.go` 文件安装到 `config/app.go` 中。 现在，由于启动了[简化代码结构](#简化代码结构)，服务提供者支持安装到 `bootstrap/providers.go` 文件中。 因此，包设置方法已修改为同时支持新旧代码结构。 未来，旧代码结构将被完全弃用。
...
func main() {
  ...
  moduleImport := setup.Paths().Module().Import() // [!code ++]
	...
	packages.Setup(os.Args). // [!code --]
  setup.Install( // [!code ++]
		Install(
			// 将 postgres 服务提供者添加到 app.go。
...
如果您想在包中同时支持新旧代码结构，请使用以下设置方法。 总之，您需要通过 `env.IsBootstrapSetup()` 函数检查项目是否使用新代码结构，然后相应应用不同的修改。
...
func main() {
  ...
  // 使用新的 setup.Paths().Module().Import() 方法获取模块导入路径
	...
	setup.Install( // [!code ++]
		// 如果使用旧代码结构，将 postgres 服务提供者添加到 app.go
		modify.When(func(_ map[string]any) bool { // [!code ++]
			return !env.IsBootstrapSetup() // [!code ++]
		...
		// 如果使用新代码结构，将 postgres 服务提供者添加到 providers.go
		modify.When(func(_ map[string]any) bool { // [!code ++]
			return env.IsBootstrapSetup() // [!code ++]
		}, modify.RegisterProvider(moduleImport, postgresServiceProvider)), // [!code ++]

	).Uninstall(
		// 如果使用旧代码结构，从 app.go 中移除 postgres 服务提供者
		modify.When(func(_ map[string]any) bool { // [!code ++]
			return !env.IsBootstrapSetup() // [!code ++]
		}, modify.GoFile(appConfigPath).
		...
		// 如果使用新代码结构，从 providers.go 中移除 postgres 服务提供者
		modify.When(func(_ map[string]any) bool { // [!code ++]
			return env.IsBootstrapSetup() // [!code ++]
		}, modify.UnregisterProvider(moduleImport, postgresServiceProvider)), // [!code ++]
	).Execute()
