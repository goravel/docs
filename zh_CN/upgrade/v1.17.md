...
4. 支持通过 `WithPath()` 函数自定义目录结构，更多详情请参阅 [自定义目录结构](../getting-started/directory-structure.md#自定义目录结构)。
...
[查看文档](../digging-deeper/http-client.md#发起请求)
...
随着 [轻量化 Goravel](#轻量版-goravel) 的发布，现在可以通过 `package:install` 和 `package:uninstall` 命令根据需要安装和卸载外观。 注意：此功能仅在你的项目使用轻量化 Goravel 构建或升级到 [新的简化代码结构](#简化代码结构) 时可用。

[查看文档](../architecture-concepts/facades.md#安装-卸载-facades)

### 邮件可通过模板发送

邮件发送支持模板，允许你使用预定义的模板发送电子邮件，以获得更好的一致性和更轻松的管理。 模板支持 http/template 和自定义驱动。

详情请 [查看文档](../digging-deeper/mail.md#使用模板)
