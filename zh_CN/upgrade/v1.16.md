...
- [修复 Distinct 与 Count 方法同时使用时没有效果的问题](#修复-distinct-与-count-方法同时使用时没有效果的问题)
- [修复 DB.Insert 方法写入数据时无法获取 nil 字段](#修复-db-insert-方法写入数据时无法获取-nil-字段)
- [修复 Orm 加载嵌套关联时的报错](#修复-orm-加载嵌套关联时的报错)
...
### 修复 Distinct 与 Count 方法同时使用时没有效果的问题

goravel/framework: v1.16.7

同时使用 `Distinct` 和 `Count` 方法时生成的 sql 不符合预期。 该修复确保使用 `Distinct` 时可以正确获取统计结果。

Issue: [#856](https://github.com/goravel/goravel/issues/856)

### 修复 DB.Insert 方法写入数据时无法获取 nil 字段

goravel/framework: v1.16.7

`facades.DB().Insert()` 无法正确处理 nil 字段。
...
// 如果第一行数据不包含 Weight 字段，则 Weight 字段将会被忽略
