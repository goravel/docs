...
Before multiple database drivers were integrated in the framework, which increased the software package size. 之前多个数据库驱动同时集成在框架中，使得软件打包体积变大，从 v1.16 开始，数据库驱动将作为独立包存在，只需安装自己需要的数据库驱动即可。
...
SQL 迁移被完全移除，请使用 Go 语言迁移。迁移步骤： Migration steps:
...
Before the `path` method returned the relative path of the root directory, for example, `path.Config("app.go") == config/app.go`. 之前 path 方法返回的是相对于根目录的路径，例如 `path.Config("app.go") == config/app.go`，从 v1.16 开始，path 方法返回的是绝对路径，例如 `path.Config("app.go") == /Users/goravel/workspace/goravel/config/app.go`；
...
Before, you needed to register the driver through the `facades.Session().Extend` method. 之前，你需要通过 `facades.Session().Extend` 方法注册驱动，从 v1.16 开始，你只需要将其添加到 `config/session.go` 配置文件中即可：
...
v1.15 之前，Queue 模块的 Redis 驱动使用 [RichardKnop/machinery](https://github.com/RichardKnop/machinery) 实现。从 v1.16 开始，Redis 驱动在 [goravel/redis](https://github.com/goravel/redis) 中提供官方支持。 Starting from v1.16, the Redis driver is officially supported in [goravel/redis](https://github.com/goravel/redis).

你依然可以在 v1.16 中使用 Machinery 驱动，但其将在 v1.17 中被移除，建议使用 Redis 驱动。下面提供两种升级方式： The following provides two upgrade methods:
